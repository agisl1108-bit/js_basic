...
* 더하기연산자는 피연산자에 따라 문자열이 하나하도 있으면 문자열 결합으로 결과가 나타나지만 빼기, 곱하기, 나누기 연산자는 숫자형으로 자동 형변환하여 인식한다.
* `"1" * 7` : 문자열 * 숫자 -> 문자를 숫자로 자동 형변환하여 결과값을 처리한다.
* `1 / 7` : 숫자 / 숫자 -> 숫자로 인식 후 결과값을 처리한다.
* `(1+7) * "2"` : (숫자+숫자) - 문자열 -> 문자를 숫자로 자동 형변환하여 결과값을 처리한다.
## 함수 종류
...
* Number(prompt()) **O** Number 괄호안은 변환대상 prompt 함수작성가능
## 복합대입연산자
...
## 숫자를 특정 기혼열에 맞게 숫자로 바꿔주는 함수
* toLocaleString('ko-kr') <- 괄호에 특정 나라 입력
## 소수점 한자리 수만 보여주기
* .toFixed(1) **(대신 문자열 처리됨)**
...
1. `get~`
    * 실제 프로그래밍 사용 시 `query~` 위조로 사용하나 레거시코드 또는 코드벤치마킹 등 사용 시 이해와 활용을 돕기 위해 간단히 일면 좋은 DOM 선택방식
...
2. `query~`
    * `querySelector`
        ...
        * 태그, 클래스 대상은 실제 HTML에 2개 이상 존재하더라도 위->아래 읽는 순서 기준 가장 먼저 인식되는 대상잉 변수에 저장됨.
    * `querySelectorAll`
