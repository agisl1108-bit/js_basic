...
// 함수준비
function func_z(){
    z = 5; // 전역변수z 값5 대입 위치 = 지역스코프(전역에서 인식못함)
    func_y(); // 지역변수 y10 값 인식을 위해 y10을 가지고 있는 함수 호출
    console.log(x+y);
}
function func_y(){
    y = 10; // 전역변수y 값10 대입 위치 = 지역스코프(전역에서 인식못함)
    console.log(x+y+z);
}
...
// 함수준비
function roomA(){
    snack = '초코파이'; // 전역변수에 문자열을 '초코파이' 대입
    console.log(`A방에서 먹는 간식 : ${snack}`); // 전역변수 호출(roomA 호출해야 가능)
}
function roomB(){ // 함수 블록 스코프 안
    let drink = '콜라';
    console.log(`B방에서 먹는 간식 : ${snack}, ${drink}`); // 전역변수 호출(roomA 호출해야 가능)
}
...
// 변수의 이름이 같아도 그 변수가 전역/지역으로 나눠서 있다면 동일설정이 가능하다.
// 변수
let wallet = '동전'; // 전역변수
// 함수호출
bank(); // 결과는? 지역변수 값 '지폐'
console.log(`wallet : ${wallet}`); // 결과는? 지역변수 값 '동전'
...
//-----------------------------------------------함수 내 전달인자(매개변수)
// 함수가 빈복될때마다 데이터를 바꾸고 싶을 때 매개변수가 필요하다.
...
school(); // 학생3 요청
// 함수준비
function school(meals='김밥'){ // 매개변수는 let, const 엾이도 알아서 변수인식함.
    // let meals = '부대찌개';
...
function kiosk(temperature ='차가운', menu, num=1){
